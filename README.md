...
## Overview
Fetch definitions into Anki from either Cambridge Dictionary (English) or ru.wiktionary.org (Russian). The add-on maps definition, examples, synonyms, part of speech, IPA (Cambridge), audio (Cambridge), syllables/stress (Wiktionary), and pictures into your note fields.
...
- Field mapping (`config.json` / addon config) keys: `word`, `definition`, `examples`, `synonyms`, `pos`, `ipa`, `audio`, `picture`. Wiktionary-only mapping lives under `wiktionary.field_map` (e.g., `syllables`).
